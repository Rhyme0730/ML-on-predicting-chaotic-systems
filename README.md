...
- **Multi-step forecasting (autoregressive rollout)**: train a one-step model $\hat{x}_{t+1}=f(x_t)$, $\hat{x}_{t+1}=f(x_t)$, then generate multi-step predictions by feeding $\hat{x}_{t+1}$ back as input to predict $\hat{x}_{t+2}, \hat{x}_{t+3}, \dots$.
